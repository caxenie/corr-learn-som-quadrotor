...
% Revision history:
%   11/24/14  AFV  Minor format changes to make arrow.m compatible with Octave
...
while (any(axnotdone)),
    ...
    try curT=get(curax,'Xform');
       ...
       % tmp=num2cell(get(curax,'View'));
        curT = [1 0 0 -0.5; 0 1 0 -0.5; 0 0 -1 9.1603; 0 0 0 1]; % curT=viewmtx(tmp{:}); % AFV: changed for compatibility with Octave 
    end
...
out = 1;  % AFV simplify for compatibility with Octave
