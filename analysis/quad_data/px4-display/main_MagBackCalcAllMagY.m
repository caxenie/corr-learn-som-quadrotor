...
mag         = data.mag(2,int);
EKF.b       = data.EKF.mag(2,int);
GT.b       = data.GT.mag(2,int);
...
k=1;
fig.p(k).title.str  = 'magnetic field magnitude in y direction $b_\mathrm{y}$';
fig.p(k).x{1}       = t;
fig.p(k).x{2}       = t;
fig.p(k).x{3}       = t;
fig.p(k).y{1}       = mag;
...
fig.p(k).ylabel.str = '$b_\mathrm{y}$ is magnetic field magnitude in Gauss';
