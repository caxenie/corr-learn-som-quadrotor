...
k=1;
fig.p(k).title.str  = 'rotated gravitational acceleration in z direction $\left[\mathbf{a}_\mathrm{rot}\right]_3$';
fig.p(k).x          = t;
fig.p(k).y{1}       = a;
fig.p(k).legend.str = {'GT RPY and $\mathbf{g}$'};
fig.p(k).ylim       = [0,12];
fig.p(k).xlabel.str = '$t$ in sec.';
fig.p(k).ylabel.str = '$\left[\mathbf{a}_\mathrm{rot}\right]_3$ is acceleration in $\frac{\mathrm{m}}{\mathrm{s}^2}$';
