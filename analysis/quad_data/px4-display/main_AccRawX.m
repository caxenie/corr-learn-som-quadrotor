...
a           = data.acc(1,int)-mean(data.acc(1,1:100));
a_filt      = data.acc_filt(1,int);
...
k=1;
fig.p(k).title.str  = 'net linear acceleration in x direction $\left[\mathbf{a}_\mathrm{net}\right]_1$';
fig.p(k).x          = t;
fig.p(k).y{1}       = a;
fig.p(k).y{2}       = a_filt;
...
fig.p(k).ylabel.str = '$\left[\mathbf{a}_\mathrm{net}\right]_1$ is acceleration in $\frac{\mathrm{m}}{\mathrm{s}^2}$';
