...
k=1;
fig.p(k).title.str  = 'rotated gravitational acceleration in y direction $\left[\mathbf{a}_\mathrm{rot}\right]_2$';
fig.p(k).x          = t;
fig.p(k).y{1}       = a;
fig.p(k).legend.str = {'GT RPY and $\mathbf{g}$'};
fig.p(k).ylim       = [-2,2];
fig.p(k).xlabel.str = '$t$ in sec.';
fig.p(k).ylabel.str = '$\left[\mathbf{a}_\mathrm{rot}\right]_2$ is acceleration in $\frac{\mathrm{m}}{\mathrm{s}^2}$';
